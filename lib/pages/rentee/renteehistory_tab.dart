...
class RenteeHistoryTab extends StatefulWidget {
  const RenteeHistoryTab({super.key});

  @override
  State<RenteeHistoryTab> createState() => _RenteeHistoryTabState();
}

class _RenteeHistoryTabState extends State<RenteeHistoryTab> {
  ...
  Future<void> _loadAdditionalData(List<Booking> bookings) async {
    // Get unique vehicle IDs and renter IDs
    final vehicleIds = bookings.map((b) => b.vehicleId).toSet();
    final renterIds = bookings.map((b) => b.renterId).toSet();
    ...
    // Load users data
    for (final renterId in renterIds) {
      if (!_userCache.containsKey(renterId)) {
        try {
          final userDoc = await FirebaseFirestore.instance
              ...
              .get();
          if (userDoc.exists) {
            _userCache[renterId] = userDoc.data() as Map<String, dynamic>;
          }
        } catch (e) {
          print('Error loading user $renterId: $e');
        }
      }
    }
  }
  ...
  List<Booking> _filterBookings(List<Booking> bookings) {
    ...
    return bookings.where((booking) {
      ...
      // Search in renter data
      final userData = _userCache[booking.renterId];
      if (userData != null) {
        ...
        Padding(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Search by vehicle, renter name, or booking details...',
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_searchQuery.isNotEmpty) ...[
                            ...
                            Text(
                              'Try searching by vehicle name, renter name, or booking details',
